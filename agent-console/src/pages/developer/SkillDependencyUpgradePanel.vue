  ...
  <section class="upgrade-panel">
    <header>
      <div>
        ...
        <h4>依赖版本</h4>
        <p>Skill 永远锁定具体 Published Version。这里仅提示新版本，不会自动升级。</p>
      </div>
    ...
    <div v-else class="upgrade-list">
      <article v-for="item in report.dependencies" :key="`${item.dependency_type}:${item.resource_id}`" class="upgrade-item">
        <div class="upgrade-main">
          <div class="upgrade-name">
            <span>{{ item.dependency_type === 'TOOL' ? 'Tool' : 'Knowledge' }}</span>
            <div><b>{{ item.display_name }}</b><small>{{ item.message }}</small></div>
          </div>
          ...
          <div class="upgrade-actions">
            ...
            <button
              v-if="item.upgrade_available"
              class="upgrade-button"
              :disabled="!item.upgrade_allowed || isLatestSelected(item)"
              @click="chooseUpgrade(item)"
            >{{ isLatestSelected(item) ? `已选择 V${item.latest.version_number}` : item.upgrade_allowed ? `升级到 V${item.latest.version_number}` : '无 USE 权限' }}</button>
          </div>
        </div>
