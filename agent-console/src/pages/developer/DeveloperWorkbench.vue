...
import SkillDependencyUpgradePanel, { type UpgradeReport } from './SkillDependencyUpgradePanel.vue'
...
const dependencyReport = ref<UpgradeReport | null>(null)
...
const addableTools = computed(() => view.value === 'EDIT' && createType.value === 'SKILL'
  ? availableTools.value.filter(item => !dependencyResourceIds.value.has(item.resource_id))
  : availableTools.value)
const addableKnowledge = computed(() => view.value === 'EDIT' && createType.value === 'SKILL'
  ? availableKnowledge.value.filter(item => !dependencyResourceIds.value.has(item.resource_id))
...
function applyDependencyUpgrade(payload: { dependencyType: 'TOOL' | 'KNOWLEDGE'; fromVersionId: string; toVersionId: string }) {
  ...
  next.push(payload.toVersionId)
  form.value[field] = next
  notice.value = `已在当前编辑表单中选择新的 ${payload.dependencyType} 版本；保存 Skill 草稿后才会生效。`
}
...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    const detail = Array.isArray(payload.detail)
      ? payload.detail.map(item => typeof item === 'object' && item && 'msg' in item ? String((item as { msg?: unknown }).msg || item) : String(item)).join('；')
...
function semanticPayload() {
  return {
    one_line_summary: form.value.oneLineSummary.trim(), when_to_use: form.value.whenToUse.trim(),
    ...(form.value.whenNotToUse.trim() ? { when_not_to_use: form.value.whenNotToUse.trim() } : {}),
    input_summary: form.value.inputSummary.trim(), output_summary: form.value.outputSummary.trim(),
    risk_level: form.value.riskLevel, read_only: form.value.readOnly,
    tags: form.value.tags.split(',').map(item => item.trim()).filter(Boolean),
    publication_scope: form.value.publicationScope, publication_subjects: [],
  }
}
function resourceConfig(): Record<string, unknown> {
  ...
  if (createType.value === 'TOOL') return {
    kind: 'NATIVE', native_name: form.value.nativeName, tool_name: form.value.toolName.trim(),
    description: form.value.description.trim() || form.value.oneLineSummary.trim(), input_schema: JSON.parse(form.value.inputSchema),
  }
...
function validateForm() {
  ...
  if (createType.value === 'TOOL') {
    if (!/^[A-Za-z][A-Za-z0-9_]{1,63}$/.test(form.value.toolName.trim())) return 'Tool Name 格式不正确。'
    try { if (JSON.parse(form.value.inputSchema)?.type !== 'object') return '输入 Schema 必须是 type=object。' } catch { return '输入 Schema 不是有效 JSON。' }
  }
...
async function createResource() {
  ...
  try {
    const common = {
      slug: form.value.slug.trim() || slugify(form.value.displayName), display_name: form.value.displayName.trim(),
      description: form.value.description.trim() || form.value.oneLineSummary.trim(), ...semanticPayload(),
    }
    const config = resourceConfig()
    if (createType.value === 'PROMPT') {
      await request('/api/v1/developer/resources/prompts', { method: 'POST', body: JSON.stringify({ ...common, template: config.template }) })
    } else if (createType.value === 'TOOL') {
      await request('/api/v1/developer/resources/native-tools', { method: 'POST', body: JSON.stringify({ ...common, native_name: config.native_name, tool_name: config.tool_name, input_schema: config.input_schema }) })
    } else {
      await request('/api/v1/developer/resources/skills', { method: 'POST', body: JSON.stringify({ ...common, skill_md: config.skill_md, tool_version_ids: config.tool_version_ids, knowledge_version_ids: config.knowledge_version_ids }) })
    }
    notice.value = `${typeLabel(createType.value)} V1 已发布。`
    await refresh(); view.value = 'MINE'; resetForm(createType.value)
  } catch (err) { error.value = err instanceof Error ? err.message : String(err) }
  finally { saving.value = false }
}
function fillVersionForm(detail: DeveloperResourceDetail) {
  createType.value = detail.resource_type as CreateType
  const config = detail.editable_config || {}
  form.value = {
    displayName: detail.display_name, slug: detail.slug, description: detail.description || '',
    oneLineSummary: detail.semantics.one_line_summary || '', whenToUse: detail.semantics.when_to_use || '', whenNotToUse: detail.semantics.when_not_to_use || '',
    inputSummary: detail.semantics.input_summary || '', outputSummary: detail.semantics.output_summary || '', riskLevel: detail.semantics.risk_level || 'LOW',
    readOnly: detail.semantics.read_only, tags: (detail.semantics.tags || []).join(', '), publicationScope: detail.semantics.publication_scope || 'PERSONAL',
    template: String(config.template || ''), nativeName: (config.native_name as FormState['nativeName']) || 'echo', toolName: String(config.tool_name || 'echo_tool'),
    inputSchema: JSON.stringify(config.input_schema || { type: 'object', properties: {} }, null, 2), skillMd: String(config.skill_md || '# 业务技能\n'),
...
async function openVersionEditor(item: CatalogItem) {
  ...
  try {
    ...
    if (!detail.editable) { selected.value = item; error.value = '当前仅支持编辑 Prompt、Skill 和平台 Native Tool。'; return }
    fillVersionForm(detail); selected.value = null; view.value = 'EDIT'; dependencyRefreshSignal.value++
  } catch (err) { error.value = err instanceof Error ? err.message : String(err) }
  finally { loading.value = false }
}
async function reloadVersionDetail() {
  ...
  resourceDetail.value = detail; fillVersionForm(detail); dependencyRefreshSignal.value++
}
...
async function publishDraft() {
  ...
  try {
    const number = activeDraft.value?.version_number
    ...
    notice.value = `V${number || ''} 已发布。旧 Agent 仍锁定原 Version，新组装时才可选择新版本。`
    await refresh(); await reloadVersionDetail()
  } catch (err) { error.value = err instanceof Error ? err.message : String(err) }
...
<template>
  <main class="developer-shell">
    <aside class="developer-sidebar">
      <div class="brand"><span>A</span><div><b>开发工作台</b><small>Agent Platform</small></div></div>
      <div class="identity"><div class="avatar">{{ principal.display_name.slice(0, 1) }}</div><div><b>{{ principal.display_name }}</b><small>{{ principal.external_user_id }}</small></div></div>
      <nav>
        ...
        <button :class="{ active: view === 'AVAILABLE' }" @click="view='AVAILABLE'; selected=null; resourceDetail=null"><span>02</span><div><b>可用资源</b><small>拥有 USE 权限的能力</small></div><em>{{ available.length }}</em></button>
        <button :class="{ active: view === 'CREATE' }" @click="view='CREATE'; selected=null; resourceDetail=null; resetForm('PROMPT')"><span>03</span><div><b>创建资源</b><small>Prompt / Skill / Tool</small></div><em>＋</em></button>
      </nav>
      <section class="sidebar-note"><b>版本规则</b><p>已发布版本不可修改；依赖升级也必须显式选择并进入新的 Skill Draft。</p></section>
      <button class="logout" @click="emit('logout')">退出登录</button>
    </aside>

    <section class="developer-content">
      <header class="topbar"><div><span>RUOYI DEVELOPER</span><b>{{ view==='MINE'?'我的资源':view==='AVAILABLE'?'可用资源':view==='CREATE'?'创建资源':editorTitle }}</b></div><button @click="refresh">刷新资源</button></header>
      ...
      <template v-if="view==='MINE' || view==='AVAILABLE'">
        <section class="hero"><div><p>{{ view==='MINE'?'MY RESOURCES':'AUTHORIZED CATALOG' }}</p><h1>{{ view==='MINE'?'持续演进你的能力资源':'复用已有企业能力' }}</h1><span>{{ view==='MINE'?'查看版本历史、创建 Draft，并主动决定依赖是否升级。':'这里只展示当前 RuoYi 身份拥有 USE 权限的资源。' }}</span></div><button v-if="view==='MINE'" class="primary" @click="view='CREATE';resetForm('PROMPT')">＋ 创建新资源</button></section>
        <section v-if="view==='MINE'" class="stats"><article><small>Prompt</small><b>{{ myCounts.PROMPT||0 }}</b></article><article><small>Skill</small><b>{{ myCounts.SKILL||0 }}</b></article><article><small>Tool</small><b>{{ myCounts.TOOL||0 }}</b></article><article><small>全部</small><b>{{ mine.length }}</b></article></section>
        <div class="filters"><input v-model="query" placeholder="搜索名称、用途或场景"/><select v-model="typeFilter"><option value="ALL">全部类型</option><option value="PROMPT">Prompt</option><option value="SKILL">Skill</option><option value="TOOL">Tool</option><option value="KNOWLEDGE">Knowledge</option><option value="MODEL">Model</option></select></div>
        <div class="resource-grid"><article v-for="item in visibleItems" :key="item.resource_id" class="resource-card"><header><span>{{ typeLabel(item.resource_type) }}</span><em>{{ sourceLabel(item.source_type) }}</em></header><h3>{{ item.display_name }}</h3><p>{{ item.one_line_summary||item.description||item.summary }}</p><div class="role"><b>它负责</b><span>{{ typeRole(item.resource_type) }}</span></div><small>适用：{{ item.when_to_use||'尚未填写' }}</small><div class="version-line"><b>V{{ item.version_number }}</b><span>{{ item.status }}</span><span>{{ item.risk_level }} 风险</span></div><footer><button @click="selected=item">查看详情</button><button v-if="view==='MINE'" class="edit" @click="openVersionEditor(item)">编辑 / 新建版本</button><strong v-else>{{ semanticReady(item)?'可复用':'说明待完善' }}</strong></footer></article><p v-if="loading" class="empty">正在加载…</p><p v-else-if="!visibleItems.length" class="empty">暂无资源。</p></div>
      </template>

      <template v-else>
        <section class="hero"><div><p>{{ view==='EDIT'?'VERSION EDITOR':'CREATE RESOURCE' }}</p><h1>{{ view==='EDIT'?editorTitle:'创建一个可复用能力' }}</h1><span>{{ view==='EDIT'?'旧版本保持不变；Skill 的依赖版本也不会自动漂移。':'先把稳定业务能力沉淀成资源。' }}</span></div><button v-if="view==='EDIT'" class="primary" @click="leaveEditor">返回我的资源</button></section>
        <div v-if="view==='CREATE'" class="type-switch"><button :class="{active:createType==='PROMPT'}" @click="switchCreateType('PROMPT')"><b>Prompt</b><span>角色与规则</span></button><button :class="{active:createType==='SKILL'}" @click="switchCreateType('SKILL')"><b>Skill</b><span>业务方法 + 依赖</span></button><button :class="{active:createType==='TOOL'}" @click="switchCreateType('TOOL')"><b>Native Tool</b><span>确定性动作</span></button></div>

        <section class="editor-layout">
          <aside class="version-sidebar"><p>{{ view==='EDIT'?'VERSION HISTORY':'CURRENT RESOURCE' }}</p><h3>{{ typeLabel(createType) }}</h3><strong>{{ typeRole(createType) }}</strong><template v-if="view==='EDIT'&&resourceDetail"><div class="safety"><b>不会覆盖旧版本</b><p>发布 V{{ editingVersionNumber }} 后，历史 Agent 仍锁定原版本。</p></div><div class="history"><article v-for="version in resourceDetail.versions" :key="version.resource_version_id"><div><b>V{{version.version_number}}</b><span :class="version.status.toLowerCase()">{{version.status==='DRAFT'?'草稿':version.status==='PUBLISHED'?'已发布':'已停用'}}</span></div><small>{{version.status==='PUBLISHED'?shortTime(version.published_at):'可继续编辑'}}</small><code>{{version.content_hash.slice(0,10)}}</code></article></div></template><ol v-else><li>说明业务价值</li><li>配置实现</li><li>发布 V1</li></ol></aside>

          <div class="editor-form">
            <section><div class="section-title"><span>01</span><div><b>这个资源是什么？</b><small>{{view==='EDIT'?'资源身份不变，本次形成新版本。':'让别人先看懂。'}}</small></div></div><div v-if="view==='EDIT'&&resourceDetail" class="identity-lock"><span><small>名称</small><b>{{resourceDetail.display_name}}</b></span><span><small>Slug</small><b>{{resourceDetail.slug}}</b></span><span><small>ID</small><b>{{resourceDetail.resource_id.slice(0,8)}}…</b></span></div><div v-else class="form-grid"><label>资源名称<input v-model="form.displayName"/></label><label>Slug<input v-model="form.slug" :placeholder="form.displayName?slugify(form.displayName):'resource-name'"/></label><label class="wide">补充说明<textarea v-model="form.description" rows="2"/></label></div><label class="wide inline">一句话能力<input v-model="form.oneLineSummary"/></label></section>
            <section><div class="section-title"><span>02</span><div><b>什么时候使用？</b><small>直接展示给 Agent 管理员。</small></div></div><div class="form-grid"><label>何时使用<textarea v-model="form.whenToUse" rows="3"/></label><label>何时不要使用<textarea v-model="form.whenNotToUse" rows="3"/></label><label>输入说明<textarea v-model="form.inputSummary" rows="3"/></label><label>输出说明<textarea v-model="form.outputSummary" rows="3"/></label></div></section>
            <section><div class="section-title"><span>03</span><div><b>{{createType==='PROMPT'?'Prompt 内容':createType==='TOOL'?'Tool 配置':'Skill 方法与依赖'}}</b><small>依赖锁定具体 Published Version。</small></div></div>
              <label v-if="createType==='PROMPT'" class="wide code">System Prompt<textarea v-model="form.template" rows="12"/></label>
              ...
              <div v-else class="skill-editor"><label class="wide code">SKILL.md<textarea v-model="form.skillMd" rows="14"/></label>
                <SkillDependencyUpgradePanel v-if="view==='EDIT'&&resourceDetail" :resource-id="resourceDetail.resource_id" :tool-version-ids="form.toolVersionIds" :knowledge-version-ids="form.knowledgeVersionIds" :refresh-signal="dependencyRefreshSignal" @upgrade="applyDependencyUpgrade" @loaded="dependencyReport=$event"/>
                <div class="dependency-section"><div><b>{{view==='EDIT'?'新增 Tool 依赖':'Tool 依赖'}}</b><small>{{view==='EDIT'?'已有依赖在上方管理版本；这里只显示尚未依赖的 Tool。':'只显示当前身份可 USE 的 Tool。'}}</small></div><div class="dependency-grid"><label v-for="item in addableTools" :key="item.version_id" :class="{selected:form.toolVersionIds.includes(item.version_id)}"><input type="checkbox" :checked="form.toolVersionIds.includes(item.version_id)" @change="toggleDependency('toolVersionIds',item.version_id)"/><span><b>{{item.display_name}} · V{{item.version_number}}</b><small>{{item.one_line_summary||item.summary}}</small></span></label><p v-if="!addableTools.length">没有可新增的 Tool。</p></div></div>
                <div class="dependency-section"><div><b>{{view==='EDIT'?'新增 Knowledge 依赖':'Knowledge 依赖'}}</b><small>{{view==='EDIT'?'已有依赖在上方管理版本；这里只显示尚未依赖的 Knowledge。':'锁定具体知识版本。'}}</small></div><div class="dependency-grid"><label v-for="item in addableKnowledge" :key="item.version_id" :class="{selected:form.knowledgeVersionIds.includes(item.version_id)}"><input type="checkbox" :checked="form.knowledgeVersionIds.includes(item.version_id)" @change="toggleDependency('knowledgeVersionIds',item.version_id)"/><span><b>{{item.display_name}} · V{{item.version_number}}</b><small>{{item.one_line_summary||item.summary}}</small></span></label><p v-if="!addableKnowledge.length">没有可新增的 Knowledge。</p></div></div>
              </div>
            </section>
            <section><div class="section-title"><span>04</span><div><b>风险与标签</b><small>资源授权范围不因创建新版本而自动变化。</small></div></div><div class="form-grid"><label>风险<select v-model="form.riskLevel"><option value="LOW">低</option><option value="MEDIUM">中</option><option value="HIGH">高</option></select></label><label v-if="view==='CREATE'">发布范围<select v-model="form.publicationScope"><option value="PERSONAL">仅自己</option><option value="OWNER_DEPT" :disabled="!principal.dept_ids.length">责任部门</option></select></label><label class="check"><input v-model="form.readOnly" type="checkbox"/>只读能力</label><label>标签<input v-model="form.tags"/></label></div></section>
            <footer v-if="view==='CREATE'" class="actions"><button class="secondary" @click="resetForm(createType)">清空</button><button class="primary dark" :disabled="saving" @click="createResource">{{saving?'发布中…':`发布 ${typeLabel(createType)} V1`}}</button></footer>
            <footer v-else class="actions version-actions"><div><b>{{activeDraft?`V${activeDraft.version_number} 当前为草稿`:`下一版本 V${editingVersionNumber}`}}</b><small>{{activeDraft?'可以反复保存。':'第一次保存创建 Draft。'}}</small></div><button class="secondary" :disabled="saving||publishing" @click="saveDraft">{{saving?'保存中…':activeDraft?`更新 V${activeDraft.version_number} 草稿`:`保存为 V${editingVersionNumber} 草稿`}}</button><button class="primary dark" :disabled="!activeDraft||saving||publishing" @click="publishDraft">{{publishing?'发布中…':activeDraft?`发布 V${activeDraft.version_number}`:'先保存草稿'}}</button></footer>
          </div>
        </section>
      </template>
    </section>
