...
interface KnowledgeJob { job_id: string; status: string; error_code?: string | null }
...
const knowledgeJob = ref<{ versionId: string; jobId: string; status: string } | null>(null)
...
function selectKind(value: Kind) { kind.value = value; error.value = ''; notice.value = ''; discovered.value = []; selectedMcpTools.value = []; mcp.value.connectionVersionId = ''; knowledgeJob.value = null; resetProductInfo(value) }
...
async function pollKnowledgeJob(versionId: string, jobId: string) {
  for (let attempt = 0; attempt < 20; attempt++) {
    await sleep(1500)
    const jobs = await request<KnowledgeJob[]>(`/api/v1/developer/external/knowledge/${versionId}/jobs`)
    const job = jobs.find(item => item.job_id === jobId)
    if (!job) continue
    knowledgeJob.value = { versionId, jobId, status: job.status }
    if (job.status === 'COMPLETED') { notice.value = 'Knowledge 索引已完成，可以直接打开 Resource Playground 做检索测试。'; return }
    if (job.status === 'FAILED') { throw new Error(`Knowledge 索引失败：${job.error_code || 'UNKNOWN'}`) }
  }
  notice.value = 'Knowledge 已进入索引任务，目前仍在处理中；稍后可在 Playground 测试，未完成时会明确提示索引尚未就绪。'
}
async function createKnowledge() {
  ...
  saving.value = true; error.value = ''; notice.value = ''; knowledgeJob.value = null
  try {
    ...
    knowledgeJob.value = { versionId, jobId: job.job_id, status: job.status }
    notice.value = `已上传 ${knowledge.value.files.length} 个文件，正在构建索引…`; emit('installed')
    await pollKnowledgeJob(versionId, job.job_id)
  } catch (err) { error.value = err instanceof Error ? err.message : String(err) } finally { saving.value = false }
}
