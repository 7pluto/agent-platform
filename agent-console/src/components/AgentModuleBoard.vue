...
const logicalCapabilities = computed(() => [...versionsByResource.value.values()].map(group => {
  const selectedItem = group.find(selected)
  return selectedItem || group[0]
}))
const currentModule = computed(() => modules.find(item => item.key === activeModule.value) || modules[0])
const sourceOptions = computed(() => [...new Set(logicalCapabilities.value
...
function upgrade(item: CatalogItem) {
  ...
  if (current && latest.version_number > current.version_number)
    emit('replace', fieldFor(item.resource_type), current.version_id, latest.version_id)
}
...
const resourceRole = (type: string) => ({
  ...
  SKILL: '业务能力包：描述完成某类任务的方法，并锁定 Tool / Knowledge 的具体版本。',
...
function dependencyDiff(item: CatalogItem) {
  ...
  return {
    added: latest.dependencies.filter(id => !before.has(id)).map(dependencyLabel),
    removed: current.dependencies.filter(id => !after.has(id)).map(dependencyLabel),
  }
}
  ...
  <div class="assembly-workbench">
    <aside class="assembly-tree">
      ...
      <button v-for="item in modules" :key="item.key" :class="{ active: activeModule === item.key }" @click="activeModule = item.key; preview = null">
        ...
        <i v-if="upgradeCountForModule(item.types)">{{ upgradeCountForModule(item.types) }} 更新</i>
      </button>
      <div class="version-rule"><b>版本规则</b><p>Agent 保存的是 Resource Version ID。新版本发布后这里只提示，不会自动替换。</p></div>
    ...
    <section class="capability-picker">
      <header>
        ...
        <div class="picker-filters"><input v-model="search" placeholder="搜索名称、用途或适用场景" /><select v-model="provider"><option value="ALL">全部来源</option><option v-for="item in sourceOptions" :key="item" :value="item">{{ sourceLabel(item) }}</option></select><select v-model="risk"><option value="ALL">全部风险</option><option value="LOW">低风险</option><option value="MEDIUM">中风险</option><option value="HIGH">高风险</option></select></div>
      </header>
      ...
      <div class="capability-grid">
        <article v-for="item in visibleCapabilities" :key="item.resource_id" :class="['capability-card', { selected: Boolean(selectedVersion(item)), incomplete: !semanticReady(item), outdated: hasUpgrade(item) }]">
          ...
          <h4>{{ item.display_name }}</h4><p>{{ capabilitySummary(item) }}</p>
          <div class="agent-role"><b>在 Agent 中</b><span>{{ resourceRole(item.resource_type) }}</span></div>
          <small>适用：{{ usageSummary(item) }}</small>
          <small v-if="item.resource_type === 'SKILL' && item.dependencies.length" class="dependency-note">当前显示版本锁定 {{ item.dependencies.length }} 项 Tool / Knowledge 依赖。</small>
      ...
      <article v-if="preview" class="capability-preview">
        ...
        <p>{{ typeLabel(preview.resource_type) }} · {{ sourceLabel(preview.source_type) }}</p><h3>{{ preview.display_name }}</h3><strong>{{ capabilitySummary(preview) }}</strong>
        <div class="preview-role"><b>它在 Agent 里负责什么？</b><span>{{ resourceRole(preview.resource_type) }}</span></div>

        <section class="version-awareness">
          <header><div><b>版本选择</b><small>显式选择 Published Version；不会自动漂移。</small></div><span v-if="previewCurrent">Agent 当前 V{{ previewCurrent.version_number }}</span><span v-else>尚未加入 Agent</span></header>
          <div class="version-options">
            <button v-for="version in previewVersions" :key="version.version_id" :class="{ active: previewCurrent?.version_id === version.version_id, latest: previewLatest?.version_id === version.version_id }" @click="chooseVersion(preview, version)"><b>V{{ version.version_number }}</b><span>{{ previewCurrent?.version_id === version.version_id ? '当前锁定' : previewLatest?.version_id === version.version_id ? '最新发布' : '历史版本' }}</span><code>{{ shortHash(version.content_hash) }}</code></button>
          </div>
          <div v-if="previewCurrent && previewLatest && previewCurrent.version_id !== previewLatest.version_id" class="version-diff">
            <div><b>V{{ previewCurrent.version_number }} → V{{ previewLatest.version_number }}</b><span>内容哈希已变化</span></div>
            <template v-if="preview.resource_type === 'SKILL'">
              <p v-if="previewDiff.added.length"><b>新增依赖：</b>{{ previewDiff.added.join('；') }}</p>
              <p v-if="previewDiff.removed.length"><b>移除依赖：</b>{{ previewDiff.removed.join('；') }}</p>
              <p v-if="!previewDiff.added.length && !previewDiff.removed.length">Skill 的依赖版本集合未变化；业务指令或其他内容可能已更新。</p>
            </template>
          </div>
        </section>
