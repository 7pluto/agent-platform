...
class ResolvedAssemblyResource:
    ...
    def __init__(
        ...
        *,
        ...
        # This is an immutable Run-time authorization snapshot.  It is only
        # allowed to be false for optional, model-invoked capabilities.
...
async def validate_agent_assembly(specification: dict, principal: Principal, action: str = "USE") -> list[ResourceVersionRecord]:
    """Compatibility wrapper for callers that only need immutable versions."""
...
async def resolve_agent_assembly(
    ...
    """Strictly resolve all bindings (used by publish and configuration flows)."""
...
async def resolve_agent_assembly_for_run(
    ...
    """Resolve a Run manifest without making optional tools a start-time gate.

    Model, Prompt, Memory Policy and Skill guidance are required to start an
    Agent, so their USE grant remains mandatory.  Tools, Knowledge and MCP
    connections are deliberately retained as denied bindings: the runtime
    checks their manifest snapshot only when the model actually calls them.
    """
...
async def _resolve_agent_assembly(
    ...
    *,
    ...
    """Resolve direct and transitive capability bindings with stable provenance."""
    deferred_types = deferred_types or set()
    registry = get_resource_registry()
    resolved: list[ResolvedAssemblyResource] = []
    pending = [(kind, version_id, "DIRECT", [f"agent.{kind.value.lower()}"])
    ...
    while pending:
        ...
        if expected_type == ResourceType.MODEL:
            # Models retain the pre-registry tables for credential testing and
            # availability state; their immutable version is resolved in runs.
        ...
        try:
            ...
            if exc.code != "RESOURCE_FORBIDDEN" or resource.resource_type not in deferred_types:
                raise
            use_allowed = False
        seen.add(version_id)
        ...
        if resource.resource_type == ResourceType.SKILL:
            ...
            for field, kind in (("tool_version_ids", ResourceType.TOOL), ("knowledge_version_ids", ResourceType.KNOWLEDGE)):
                dependencies = config.get(field, [])
                if not isinstance(dependencies, list):
                    raise ApiError(422, "INVALID_SKILL_CONFIG", f"{field} must be a list")
                pending.extend((kind, _uuid(item, f"skill.{field}"), "TRANSITIVE", path + [f"skill.{field}"])
        ...
        if resource.resource_type == ResourceType.TOOL and resource.config.get("kind") == "MCP":
            ...
            pending.append((ResourceType.MCP_CONNECTION, _uuid(connection_id, "tool.connection_version_id"), "TRANSITIVE", path + ["tool.connection_version_id"]))
    return resolved
